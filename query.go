...
func GetRecommendationsURL(w http.ResponseWriter, r *http.Request) {
    path := chi.URLParam(r, "*")
    req := requests.New(requests.WithBaseURL("https://api.spotify.com/v1/"), requests.WithRequestURL("recommendations?"+path))
    log.Printf("%s%s",req.BaseURL,req.RequestURL)
    requests.GetRequest(a, req)
    _, err := w.Write(req.Response)
    if err != nil {
        http.Error(w,err.Error(),http.StatusInternalServerError)
    }
}
...
func GetTop(w http.ResponseWriter, r *http.Request) {
    targetType := chi.URLParam(r,"*")
    req := requests.New(requests.WithBaseURL("https://api.spotify.com/v1/"), requests.WithRequestURL("me/top/"+targetType))
    log.Printf(req.BaseURL+req.RequestURL)
    requests.GetRequest(a,req)
    _,err := w.Write(req.Response)
    if err != nil {
        http.Error(w,err.Error(),http.StatusInternalServerError)
...
func GetRecommendations(w http.ResponseWriter, r *http.Request) {
    ...
    _ = requests.ParamRequest(a, recRequest,
        recommendations.ListParams(data.SeedValues),
        requests.Limit(data.Limit),
        recommendations.PercentParams(data.PercentValues),
        recommendations.IntParams(data.IntValues))
    w.WriteHeader(http.StatusOK)
